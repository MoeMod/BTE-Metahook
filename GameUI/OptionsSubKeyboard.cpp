...
void COptionsSubKeyboard::OnCommand( const char *command )
{
	...
	else if (!m_pKeyBindList->IsCapturing() && !stricmp(command, "OpenZBSKDlg"))
	{
		if (!m_hSkillKeyPopupDlg.Get())
			m_hSkillKeyPopupDlg = new CSkillKeyPopupDlg(this);

		m_hSkillKeyPopupDlg->Activate();
	}
	else
	{
