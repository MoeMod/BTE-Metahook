...
bool CWin32Font::Create(const char *windowsFontName, int tall, int weight, int blur, int scanlines, int flags)
{
	...
	if (!stricmp(windowsFontName, gConfigs.szCustomFontName))
	{
		if (!(m_iFlags & vgui::ISurface::FONTFLAG_CUSTOM))
			m_iFlags |= vgui::ISurface::FONTFLAG_CUSTOM;
	}
	...
	m_hFont = ::CreateFontA(tall, 0, 0, 0, m_iWeight, flags & vgui::ISurface::FONTFLAG_ITALIC, m_bUnderlined, flags & vgui::ISurface::FONTFLAG_STRIKEOUT, charset, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, m_bAntiAliased ? ANTIALIASED_QUALITY : NONANTIALIASED_QUALITY, DEFAULT_PITCH | FF_DONTCARE, windowsFontName);
