...
int LoadBMP(const char *szFilename, byte *buffer, int bufferSize, int *width, int *height)
{
	...
	if (!file)
	{
		LogToFile("LoadBMP() : Fail Load %s (File not found)", szFilename);
		return FALSE;
	}
	...
	if (!g_pFileSystem->Read(&bmfHeader, sizeof(bmfHeader), file))
	{
		*width = 0;
		*height = 0;
		LogToFile("LoadBMP() : Fail Load %s (Error header)", szFilename);
	...
	if (bmfHeader.bfType == DIB_HEADER_MARKER)
	{
		...
		if (!g_pFileSystem->Read(pDIB, dwBitsSize, file))
		{
			...
			*height = 0;
			...
			g_pFileSystem->Close(file);
			LogToFile("LoadBMP() : Fail Load %s (incomplete file)", szFilename);
			return FALSE;
		}
