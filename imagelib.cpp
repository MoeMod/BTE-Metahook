...
bool Image_LoadImage(char *pName,imagedata_t *pImageData,int *TexID)
{
	...
	if (!Q_stricmp(szExt, "bmp"))
	{
		...
		vgui::surface()->DrawSetTextureRGBA(*TexID, data, pImageData->width, pImageData->height, true, false);
...
bool LoadTextureTga( const char *name, int TexID, int &w, int &h)
{
	...
	vgui::surface()->DrawSetTextureRGBA(TexID, targa_rgba, targa_header.width, targa_header.height, true, false);

	w = targa_header.width;
	h = targa_header.height;
    ...
    for(int n = 0; n < readtimes; ++n)
    {
        for (iy = 0; iy < readlines; ++iy)
        {
            for (ix = 0; ix < bmx; ++ix)
            {
				...
				if (caTmp[1] == -1 && caTmp[0] == 0 && caTmp[2] == 0)
				{
					caTmp[1] = 0;
					caTmp[3] = 0;
				}

				pdat[dwDataOff*4] = caTmp[0];
				pdat[dwDataOff*4+1] = caTmp[1];
				pdat[dwDataOff*4+2] = caTmp[2];
				pdat[dwDataOff*4+3] = caTmp[3];
            }
        }
        picy += readlines;
...
int LoadBMP2(const char *szFilename, byte *buffer, int bufferSize, int &width, int &height)
{
	FileHandle_t file = g_pFileSystem->Open(szFilename, "rb");
	...
	if (bmfHeader.bfType == DIB_HEADER_MARKER)
	{
		...
		for (int j = 0; j < lpbmi->bmiHeader.biHeight; j++)
		{
			for (int i = 0; i < lpbmi->bmiHeader.biWidth; i++)
			{
				...
				int gray = dst[0] * 0.299 + dst[1] * 0.587 + dst[2] * 0.114;
...
void LoadTexture(char *name, int &id, wrect_t *rc)
{
	...
	imagedata_s data;
	Image_LoadImage(name, &data, &id);

	if (rc)
	{
		rc->right = data.width;
		rc->bottom = data.height;
	}
}