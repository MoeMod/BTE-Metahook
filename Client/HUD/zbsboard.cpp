...
{
	return g_HudZBSBoard;
}

#define FLASH_TIME (1.23 / 2.2)

void CHudZBSBoard::Init(void)
{
	m_iFlags |= HUD_ACTIVE;
}

void CHudZBSBoard::VidInit(void)
{
	m_iBackground = Hud().m_TGA.GetTGA("mode\\zbs\\ZBSboard");
	m_iTeamnumber = Hud().m_TGA.GetTGA("mode\\zbs\\Teamnumber");
	m_iSelfnumber = Hud().m_TGA.GetTGA("mode\\zbs\\Selfnumber");
	m_iToprecord = Hud().m_TGA.GetTGA("mode\\zbs\\Toprecord");
...
void CHudZBSBoard::Draw(float time)
{
	if (g_iMod != MOD_ZBS)
		return;
	...
	x = (ScreenWidth - g_MHTga[m_iBackground].width) / 2;
	y = 5;
	...
	if (m_flEndFlashTime && Hud().m_flTime < m_flEndFlashTime && m_flFlashSize > 1)
	{
		...
		if (flFlashTime > 0)
		{
			...
			//gEngfuncs.Con_Printf("m_flFlashSize:%f, :%f\n", m_flFlashSize, (m_flEndFlashTime - Hud().m_flTime));

			float flHasShadeTime = Hud().m_flTime - m_flStartShadeTime;
			...
			if (m_iShadeType == 0)
			{
				if (m_flEndShadeTime && Hud().m_flTime < m_flEndShadeTime)
				{
				...
				{
					m_flStartShadeTime = Hud().m_flTime;
...
void CHudZBSBoard::BuildNumberRC(wrect_t *rgrc, int w, int h)
{
	int nw = 0;
	int nh = 0;
...
void CHudZBSBoard::DrawTeamNumber(int n, int x, int y, float size)
{
	TEXD_BindTexture(g_MHTga[m_iTeamnumber].texid);
	TEXD_DrawTexture(x, y, 230, 24, &m_rcTeamnumber[n], size);
}

int CHudZBSBoard::GetNumWidth(int n, int w, float size)
{
	w *= size;
...
int CHudZBSBoard::DrawTeamNumbers(int n, int x, int y, float size)
{
	int width = 23 + 4;
	width *= size;
...
void CHudZBSBoard::DrawSelfNumber(int n, int x, int y, float size)
{
	TEXD_BindTexture(g_MHTga[m_iSelfnumber].texid);
	TEXD_DrawTexture(x, y, 180, 19, &m_rcSelfnumber[n], size);
}

int CHudZBSBoard::DrawSelfNumbers(int n, int x, int y, float size)
{
	int width = 18 + 4;
	width *= size;
...
void CHudZBSBoard::DrawTRecordNumber(int n, int x, int y, float size)
{
	TEXD_BindTexture(g_MHTga[m_iToprecord].texid);
	TEXD_DrawTexture(x, y, 140, 14, &m_rcToprecord[n], size);
}

int CHudZBSBoard::DrawTRecordNumbers(int n, int x, int y, float size)
{
	int width = 14 + 4;
	width *= size;
...
void CHudZBSBoard::SetPlayerScore(int score, int flash)
{
	m_iPlayerScore = score;

	if (flash)
	{
		m_flStartFlashTime = Hud().m_flTime;
...
void CHudZBSBoard::SetTeamScore(int score)
{
	m_iTeamScore = score;
}