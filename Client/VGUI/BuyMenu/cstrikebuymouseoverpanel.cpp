...
CSBuyMouseOverPanel::CSBuyMouseOverPanel(vgui::Panel *parent, const char *panelName) : BaseClass(parent, panelName)
{
...
void CSBuyMouseOverPanel::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	BaseClass::ApplySchemeSettings(pScheme);

	vgui::Label *info_label[] = { pricelabel, calibrelabel, clipcapacitylabel, rateoffirelabel, weightloadedlabel };
	vgui::Label *info[] = { price, calibre, clipcapacity, rateoffire, weightempty };
...
void CSBuyMouseOverPanel::UpdateWeapon(const char *weapon)
{
	...
	for (int i = 0; i < 5; ++i)
	{
		info[i]->SetText(GetWeaponInfoFormat(i, weapon));
	}
}