...
void UI_Init(void)
{
	g_iUI_Mouse = vgui::surface()->CreateNewTextureID();
	vgui::surface()->DrawSetTextureFile(g_iUI_Mouse,"gfx\\ui\\mouse\\mouse",1,1);
	g_iUI_Play = vgui::surface()->CreateNewTextureID();
	vgui::surface()->DrawSetTextureFile(g_iUI_Play,"gfx\\ui\\music_player\\play",1,1);
	g_iUI_Stop = vgui::surface()->CreateNewTextureID();
	vgui::surface()->DrawSetTextureFile(g_iUI_Stop,"gfx\\ui\\music_player\\stop",1,1);
	g_iUI_Pause = vgui::surface()->CreateNewTextureID();
	vgui::surface()->DrawSetTextureFile(g_iUI_Pause,"gfx\\ui\\music_player\\pause",1,1);
	g_iUI_Next = vgui::surface()->CreateNewTextureID();
	vgui::surface()->DrawSetTextureFile(g_iUI_Next,"gfx\\ui\\music_player\\next",1,1);
	g_iUI_Prev = vgui::surface()->CreateNewTextureID();
	vgui::surface()->DrawSetTextureFile(g_iUI_Prev,"gfx\\ui\\music_player\\prev",1,1);
...
void UI_Draw(void)
{
	...
	FMOD_Channel_GetPosition(g_fmod.channel,&iPos,FMOD_TIMEUNIT_MS);
	...
	if(g_iMini)	
	{
		...
		if(g_iMenuActive)
		{
			...
			vgui::surface()->DrawSetTexture(g_iUI_Mouse);
			vgui::surface()->DrawTexturedRect(iMouseX,iMouseY,iMouseX+32,iMouseY+32);
		}
		return;
	}
	...
	gEngfuncs.pfnDrawConsoleString(StartX+30,StartY-Hud().m_scrinfo.iCharHeight - 3,szPrint);
	...
	{
		if(g_iMouseEvent & ME_LEFTCLICK) vgui::surface()->DrawSetColor(150,150,150,255);
		else vgui::surface()->DrawSetColor(255,255,255,255);
		if(IsClick())
		{
			char cmd[64];
			g_iMusicIndex --;
	...
	else vgui::surface()->DrawSetColor(255,255,255,80);
	vgui::surface()->DrawSetTexture(g_iUI_Prev);
	vgui::surface()->DrawTexturedRect(StartX,StartY,StartX+32,StartY+32);


	StartX += 40;
	if(iMouseX >StartX && iMouseX<StartX+32 && iMouseY>StartY && iMouseY<StartY +32)
	{
		if(g_iMouseEvent & ME_LEFTCLICK) vgui::surface()->DrawSetColor(255,255,255,200);
		else vgui::surface()->DrawSetColor(255,255,255,255);
	...
	vgui::surface()->DrawSetTexture(g_iPlayStatus==3?g_iUI_Play:g_iUI_Pause);
	vgui::surface()->DrawTexturedRect(StartX,StartY,StartX+32,StartY+32);
	...
	vgui::surface()->DrawSetTexture(g_iUI_Stop);
	vgui::surface()->DrawTexturedRect(StartX,StartY,StartX+32,StartY+32);
	...
	vgui::surface()->DrawSetTexture(g_iUI_Next);
	vgui::surface()->DrawTexturedRect(StartX,StartY,StartX+32,StartY+32);
	...
	if(g_iMusicCount)
	{
		...
		{
			// Draw BG
			if(iMouseX >StartX && iMouseX<StartX+300 && iMouseY>StartY+(i-1)*(Hud().m_scrinfo.iCharHeight+5)-2 && iMouseY<StartY+(i-1)*(Hud().m_scrinfo.iCharHeight+5)-2 +Hud().m_scrinfo.iCharHeight)
			{
				if(g_iMouseEvent & ME_LEFTCLICK) gEngfuncs.pfnFillRGBA(StartX,StartY+(i-1)*(Hud().m_scrinfo.iCharHeight+5)-2, 320,Hud().m_scrinfo.iCharHeight,100,100,100,100);
				else gEngfuncs.pfnFillRGBA(StartX,StartY+(i-1)*(Hud().m_scrinfo.iCharHeight+5)-2, 320,Hud().m_scrinfo.iCharHeight,255,255,100,100);
			...
			else gEngfuncs.pfnFillRGBA(StartX,StartY+(i-1)*(Hud().m_scrinfo.iCharHeight+5)-2, 320,Hud().m_scrinfo.iCharHeight,100,100,100,100);
			...
			if(g_iMusicIndex == i && g_iPlayStatus && g_iPlayStatus<3)
			{
				...
				gEngfuncs.pfnDrawConsoleString(StartX+320 - 50,StartY+(i-1)*(Hud().m_scrinfo.iCharHeight+5),playeffect);
			}

			sprintf(szPrint,"%02d.%s",i,g_szMusicList[i]);
			gEngfuncs.pfnDrawSetTextColor(1,1,1);
			gEngfuncs.pfnDrawConsoleString(StartX,StartY+(i-1)*(Hud().m_scrinfo.iCharHeight+5),szPrint);
		}
	}
	// Draw Mouse
	gEngfuncs.pTriAPI->RenderMode( kRenderTransTexture );
...
unsigned __stdcall PlayAudio_FMOD(void *p)
{
	...
	if(iPlayType == 0) // stop
	{
		...
		int playing = 0;
		FMOD_System_GetChannelsPlaying(g_fmod.system, &playing);
		if (playing)
		{
			FMOD_Sound_Release(g_fmod.sound);
			g_fmod.sound = NULL;
		}
		return 0;
	}
	else if(iPlayType == 1 || iPlayType == 2) // play / loop
	{
		...
		if (FMOD_System_CreateSound(g_fmod.system, fullPath, FMOD_SOFTWARE, 0, &g_fmod.sound) != FMOD_OK)
		{
			g_iPlayStatus = 0;
			g_iMusic_Length = 0;
		...
		FMOD_Sound_SetMode(g_fmod.sound, loop ? FMOD_LOOP_NORMAL : FMOD_LOOP_OFF);
		if (FMOD_System_PlaySound(g_fmod.system, FMOD_CHANNEL_FREE, g_fmod.sound, 0, &g_fmod.channel) != FMOD_OK)
		{
		...
		if (FMOD_Channel_SetCallback(g_fmod.channel,Audio_Channel_CallBack) == FMOD_OK) 
		{
			FMOD_System_Update(g_fmod.system);
		}
		unsigned int i;
		FMOD_Sound_GetLength(g_fmod.sound,&i,FMOD_TIMEUNIT_MS);
		g_iMusic_Length = i;
	...
	else if(iPlayType == 3) // pause
	{
		...
		if (playing)
		{
			if (FMOD_Channel_SetPaused(g_fmod.channel,true) != FMOD_OK)
			{
	...
	{
		if (FMOD_Channel_SetPaused(g_fmod.channel,false) != FMOD_OK)
		{
...
void PlayAudio_Init(void)
{
	...
	if (FMOD_System_Create(&g_fmod.system) != FMOD_OK)
		return;

	if (FMOD_System_Init(g_fmod.system, 1, FMOD_INIT_NORMAL, NULL) != FMOD_OK)
		return;


	g_fmod.initialied = true;
}
...
void PlayAudio_Shutdown(void)
{
	if (g_fmod.initialied)
	{
		FMOD_System_Close(g_fmod.system);
		FMOD_System_Release(g_fmod.system);
	}
...
void PlayAudio_Frame(void)
{
	...
	float volume = 0.0;
	FMOD_Channel_GetVolume(g_fmod.channel, &volume);

	float cvar_volume = MP3Volume->value * 2;

	if (volume != cvar_volume)
		FMOD_Channel_SetVolume(g_fmod.channel, max(0, cvar_volume));
